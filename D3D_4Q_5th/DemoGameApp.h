...
public :
	...
	ComPtr<ID3D11RenderTargetView> m_pHDRRenderTargetView = nullptr;
	ComPtr<ID3D11DepthStencilView> m_pDepthStencilView = nullptr; // Depth/Stencil view
	ComPtr<ID3D11DepthStencilState> m_pDepthOnWriteOnState = nullptr;
	...
	ComPtr<ID3D11ShaderResourceView> m_pHDRShaderResourceView = nullptr;
	ComPtr<ID3D11SamplerState> m_pSamplerStateLinear = nullptr; // Sampler State Linear
	ComPtr<ID3D11SamplerState> m_pSamplerStateClamp = nullptr; // Sampler State Clamp
	ComPtr<ID3D11Texture2D> m_pHDRRenderTarget = nullptr;
	...
	float m_MonitorMaxNits;
	float m_Exposure = -2.2;
	DXGI_FORMAT m_Format;
	HDR_Quad m_HDRQuad;
	GBuffer m_GBuffer;
	LightBuffer m_LightBuffer;

	ComPtr<ID3D11DepthStencilState> m_pDepthOffWriteOffState = nullptr;
	ComPtr<ID3D11DepthStencilState> m_pDepthOnWriteOffState = nullptr;
	ComPtr<ID3D11DepthStencilState> m_pLightVolumDSS = nullptr;
	ComPtr<ID3D11RasterizerState> m_pcullnonestate = nullptr;
	...
	bool CheckHDRSupporAndMaxNits(float& outmaxNits, DXGI_FORMAT& outFormat);
