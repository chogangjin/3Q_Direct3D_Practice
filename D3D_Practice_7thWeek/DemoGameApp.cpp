...
void DemoGameApp::Render()
{
	...
	for (int i = 0; i < m_ZeldaModel.m_Meshes.size(); i++)
	{
		...
		m_pDeviceContext->VSSetConstantBuffers(3, 1, m_pBoneMatrixBuffer.GetAddressOf());
...
bool DemoGameApp::InitScene()
{
	...
	HR_T(m_pDevice->CreateBuffer(&cbDesc, nullptr, m_pBoneMatrixBuffer.GetAddressOf()));
	//m_pDeviceContext->UpdateSubresource(m_pBoneMatrixBuffer.Get(), 0, nullptr, &m_ZeldaModel.m_SkeletonPose.modelMatrix[0], 0, 0);
	m_pDeviceContext->VSSetConstantBuffers(3, 1, m_pBoneMatrixBuffer.GetAddressOf());
