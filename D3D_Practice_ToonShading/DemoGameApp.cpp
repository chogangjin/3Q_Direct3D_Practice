...
void DemoGameApp::Render()
{
	...
	//depth/stencil state 및 rasterizer 초기화
	...
	m_pDeviceContext->UpdateSubresource(m_pConstantBuffer.Get(), 0, nullptr, &cbuffer, 0, 0);
	m_pDeviceContext->PSSetShaderResources(0, 1, m_pRampTexture.GetAddressOf());
	m_ZeldaModel.Draw(m_pDeviceContext.Get());

	// model outline
	m_pDeviceContext->OMSetDepthStencilState(m_pOutLineDepthStencilState.Get(), 0);
	m_pDeviceContext->RSSetState(m_pOutLineRasterizerState.Get());
	m_pDeviceContext->IASetInputLayout(m_pOutLineInputLayout.Get());
	m_pDeviceContext->VSSetShader(m_pOutLineVertexShader.Get(), nullptr, 0);
	m_pDeviceContext->PSSetShader(m_pOutlinePixelShader.Get(), nullptr, 0);
	m_pDeviceContext->UpdateSubresource(m_pConstantBuffer.Get(), 0, nullptr, &cbuffer, 0, 0);
	cbuffer.World = XMMatrixTranspose(m_WorldMatrix);
	m_ZeldaModel.Draw(m_pDeviceContext.Get());
...
bool DemoGameApp::InitD3D()
{
	...
	m_pDevice->CreateDepthStencilState(&depthStencilDesc, m_pOutLineDepthStencilState.GetAddressOf());
	...
	rasterizerDesc.DepthClipEnable = true;
	rasterizerDesc.FrontCounterClockwise = true;
	m_pDevice->CreateRasterizerState(&rasterizerDesc, m_pOutLineRasterizerState.GetAddressOf());
...
void DemoGameApp::SetCube()
{
	...
	ComPtr<ID3DBlob> OutLineVertexBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"OutLIneVertexShader.hlsl", "main", "vs_4_0", OutLineVertexBuffer.GetAddressOf()));
	HR_T(m_pDevice->CreateVertexShader(OutLineVertexBuffer->GetBufferPointer(),
		OutLineVertexBuffer->GetBufferSize(), NULL, m_pOutLineVertexShader.GetAddressOf()));
	...
	D3D11_INPUT_ELEMENT_DESC OutLineInputLayout[] =
	{
		{"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,  D3D11_INPUT_PER_VERTEX_DATA, 0},
		{"NORMAL"  , 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0}
	};
	...
	HR_T(m_pDevice->CreateInputLayout(
		OutLineInputLayout,
		ARRAYSIZE(OutLineInputLayout),
		OutLineVertexBuffer->GetBufferPointer(),
		OutLineVertexBuffer->GetBufferSize(),
		m_pOutLineInputLayout.GetAddressOf()));
	...
	ComPtr<ID3DBlob> outlinePixelShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"OutlLinePixelShader.hlsl", "main", "ps_4_0", outlinePixelShaderBuffer.GetAddressOf()));
	HR_T(m_pDevice->CreatePixelShader(
		outlinePixelShaderBuffer->GetBufferPointer(),
		outlinePixelShaderBuffer->GetBufferSize(), NULL, m_pOutlinePixelShader.GetAddressOf()));
	...
	ComPtr<ID3D11Resource> ramptexture = nullptr;
	HR_T(DirectX::CreateWICTextureFromFile(m_pDevice.Get(), L"../Resources/RampTexture.png", ramptexture.GetAddressOf(), m_pRampTexture.GetAddressOf()));
	...
	samplerStateDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT; // D3D11_FILTER_MIN_MAG_MIP_LINEAR 대신 POINT로 하기
	...
	m_ZeldaModel.LoadModel(m_handleWindow, m_pDevice.Get(), m_pDeviceContext.Get(), "../Resources/Teapot.fbx");
