...
bool StaticFBX::LoadModel(HWND hwnd, ID3D11Device* device, ID3D11DeviceContext* devicecontext, std::string _filepath)
{
	//Assimp 임포트 옵션
...
void StaticFBX::Draw(ConstantBuffer* pCBuffer, ID3D11Buffer* pConstantBuffer)
{
...
void StaticFBX::Close()
{

}
void StaticFBX::processNode(aiNode* node, const aiScene* scene)
{
...
Mesh StaticFBX::processMesh(aiMesh* mesh, const aiScene* scene)
{
...
std::vector<Texture> StaticFBX::loadMaterialTextures(aiMaterial* material, aiTextureType type, std::string typeName, const aiScene* scene)
{
	...
	for (UINT i = 0; i < material->GetTextureCount(type); i++) //텍스쳐 개수만큼 반복
	{
		...
		if (!skip)
		{
			...
			else
			{
				...
				if (std::string fileformat = fixedFilePath.substr(fixedFilePath.find_last_of(".") + 1, fixedFilePath.size()); fileformat == "tga")
				{
					DirectX::ScratchImage image;
					DirectX::TexMetadata metadata;
					filename = filepath + '/' + fixedFilePath;
					std::wstring wfilename = std::wstring(filename.begin(), filename.end());
					hr = DirectX::LoadFromTGAFile(wfilename.c_str(), &metadata, image);

					ComPtr<ID3D11Resource> textureresource = nullptr;
...
ID3D11ShaderResourceView* StaticFBX::loadEmbeddedTexture(const aiTexture* embeddedTexture)
{
	HRESULT hr;
